...
class Heuristic(object):
    ...
    def attacks(self):
        ...
        # estao atacando pelo menos uma rainha, caso a condicao
        # seja aceita, podemos retornar o valor

        maxColision.append(self.attackMD(self.state, qP).count(1))

        # if self.colision.count(1) == (len(qP) - 1):
        #     return self.colision.count(1)

        maxColision.append(self.attackSD(self.state, qP).count(1))

        # if self.colision.count(1) == (len(qP) - 1):
        #     return self.colision.count(1)

        maxColision.append(self.attackH(self.state, qP).count(1))

        return max(maxColision)
    ...
    def attackMD(self, state, qP):  # Rainhas na diagonal principal
        ...
        # Se abs(qP[c] - c) = len(qP) nao conta colisao,
        # pois significa que a diagonal principal e ela mesma.
        # EX : tabuleiro 4x4 => rainha esta na posicao C = 3 e L = 0
